/***************************************************************************************************
* FILE		: mat-mat-mul-mmap-dgemm-offload.c
*
* AUTHOR	: K V SRINATH
*
* INPUT		: Matrix Size
...
#include<offload.h>
int k=0;
__attribute__((target(mic))) double alpha=1.0;
__attribute__((target(mic))) double beta=0.0;
...
__attribute__((target(mic))) void doMult(double *ma,double *mb,double *mr,int size)
{
	double sum=0.0;
	int i,j,k;
...
__attribute__((target(mic))) void call_dgemm(double *restrict a,double * restrict b,double * restrict c ,int size)
{
...
int main(int argc,char *argv[])
{
	...
	in(mapA:length(size*size))  \
	in(mapB:length(size*size))  \
	out(mapR:length(size*size))  
