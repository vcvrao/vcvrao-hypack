 ...
 * 	    (as many as user specifies) using condition variable
...
typedef struct
{
  ...
  pthread_cond_t *queue_full;//queue full condition variable
  pthread_cond_t *queue_empty;//queue empty condition variable
} queue;
  ...
  while (loopcnt)
    {
     ...
     while(q->contains == queue_size-1)//queue is full
...
consumer (void *arg)
{
  ...
  while (loopcnt)
    {
      pthread_mutex_lock (q->mutex);//to obtain mutex on the queue
      while(q->contains == 0)//queue is empty
...
main (int argc, char *argv[])
{
  ...
  q->mutex = (pthread_mutex_t *) malloc (sizeof (pthread_mutex_t));
  pthread_mutex_init (q->mutex, NULL);//to initialize the queue mutex
  q->queue_full = (pthread_cond_t *) malloc (sizeof (pthread_cond_t));
  pthread_cond_init(q->queue_full,NULL);//to initialize the queue full condition variable
  q->queue_empty = (pthread_cond_t *) malloc (sizeof (pthread_cond_t));
  pthread_cond_init(q->queue_empty,NULL );//to initialize the queue empty condition variable
  ...
  for (i = 0; i < num_threads; i++)
   ...
   if(argc == 4)
   {
    ...
    printf("Execution time (micro sec) : %lf",timetaken);
    printf("\nExecution time (sec)       : %lf\n",timetaken/1000000);
