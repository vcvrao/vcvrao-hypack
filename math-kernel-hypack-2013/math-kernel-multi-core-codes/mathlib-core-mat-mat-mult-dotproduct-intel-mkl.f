...
c
c Example 1.7           : mathlib-core-mat-mat-mult-dotproduct-intel-mkl.f
c
c Objective 	        : Write matrix-matrix multiplication program with dot 
c                         product inner loop, and use IBM ESSL/ESSL-SMP BLAS 
c                         libraries and compiler optimizations to extract the 
c                         performance. For linking, use -lessl/-lesslsmp for
c                         using ESSL/ESSL-SMP libraries respectively.
c
c Input                 : The size of two matrices MatrixA and MatrixB.
c
