...
Descritption    : The Simple example program to demonstart Matrix Matrix Multiplication 
                  using Multi GPU (OpenCL+OpenMP) and Device Shared Memory. 
...
const char *kernel_source_path = "Mat_Mat_Mult_Openmp_Opencl_SharedMemory.cl";
...
int main(int argc, char **argv)
{
	...
	int 		row_count, col_count; /* loop control variables */
	 ...
	 height_a = width_b = rows;
         width_a = height_b = cols;
          ...
          /* 3. Initializing Host output matrix with zero  */
	 ...
	 /** 4. Setting the exeution environment for openCL kernel : step (5-9)**/
	 ...
	 /** 10. function to perform matrix multiplication on GPU : ( STEPS : 11- 23**/
	...
	*/
        ...
        /* 24. Check the GPU results against CPU result **/
	...
	double			gflops;
 	...
 	/*** 13. allocating memory for device variables */
        ...
        assert((time = (cl_event *) malloc (sizeof(cl_event) * num_devices)) != NULL);
         ...
         /*** 14. OpenMP : Create the team of threads */
		...
		if ( omp_get_thread_num() == 0)
                ...
                /******** 15  . Create the command queue for requested devices *********/
                ...
                /******* 16. Allocate and initialize memory buffer on each device  ****************/
                	...
                	/** Allocate memory for a variable to hold Num Rows  **/
                	cl_mem d_rows =  clCreateBuffer(context,CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_int), (void*) &width_a, &err);
                	checkStatus( "Failed to create device output  buffer   ",err);

			/** Allocate memory for a variable to hold Num Cols **/
                	cl_mem d_cols =  clCreateBuffer(context,CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(cl_int), (void*) &width_b, &err);
                	checkStatus( "Failed to create device output  buffer   ",err);


                /******* 17. Create the kernel **************/
                kernel[omp_get_thread_num()] = clCreateKernel ( program, "matMatMultKernel", &err);
                checkStatus(" Create kernel failed ",err);

                /******* 18. Set the kernel arguments ********/
                err |= clSetKernelArg( kernel[omp_get_thread_num()], 0, sizeof(cl_mem), (void *) &d_output[omp_get_thread_num()]);
                err |= clSetKernelArg( kernel[omp_get_thread_num()], 1, sizeof(cl_mem), (void *) &d_a[omp_get_thread_num()]);
                err |= clSetKernelArg( kernel[omp_get_thread_num()], 2, sizeof(cl_mem), (void *) &d_b[omp_get_thread_num()]);
                err |= clSetKernelArg( kernel[omp_get_thread_num()], 3, sizeof(cl_float) * BLOCK_SIZE * BLOCK_SIZE, 0);
                err |= clSetKernelArg( kernel[omp_get_thread_num()], 4, sizeof(cl_float) * BLOCK_SIZE * BLOCK_SIZE, 0);
                err |= clSetKernelArg( kernel[omp_get_thread_num()], 5, sizeof(cl_mem), (void *) &d_rows);
                err |= clSetKernelArg( kernel[omp_get_thread_num()], 6, sizeof(cl_mem), (void *) &d_cols);
                checkStatus( "Set  kernel arguments failed ",err);


		/** 19. Setting the Global & Local work group size **/
		...
		clFinish(cmd_queue[omp_get_thread_num()]);
                ...
                /****** 21. Read Results from the device ******/
                err = clEnqueueReadBuffer(cmd_queue[omp_get_thread_num()], d_output[omp_get_thread_num()], CL_TRUE, 0, width_b * partition_size * sizeof(cl_float),
...
//	 printf("\n\n Total kernel execution time for Matrix Matrix Multiplication \t: %.5f sec ",total_time);
...
int checkResult (float *h_a, float *h_b,float *output, int rows, int cols)
{
	...
	float  errorNorm = 0.0;
        ...
        temp_out = (float *)malloc( sizeof(float) * rows * rows);
