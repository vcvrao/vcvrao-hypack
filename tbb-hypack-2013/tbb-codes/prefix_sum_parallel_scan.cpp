...
*
*
* File	  : prefix_sum_parallel_scan.cpp
* Desc   : Computes the prefix sum of a randomly generated vector upto n-2 terms
* Input  : <vector_size> , <grain_size>
* Output : The prefix sum and the timetaken for the operation to complete
*
* Created             : August-2013
*
...
size_t grain_size;//grain_size for the parallel_scan 
...
public:
  ...
  {
    //actual prefix sum computation in accordance with the grain_size
    int temp = sum;
...
{
  Body body (y,x);
  parallel_scan (blocked_range < int >(0, n, grain_size), body);
  return body.get_sum ();
}
...
main (int argc, char *argv[])
{
  ...
  int *vector_y;
  long vector_size;
  if (argc == 3)//if the user inputs vector_size and the grain_size
    {
      vector_size = atoi (argv[1]);
      grain_size = atoi (argv[2]);
    }
  else if (argc == 2) // if the user inputs on the vector size
    {
      cout << "assuming the grainsize to be 10000" << endl;
      vector_size = atoi (argv[1]);
      grain_size = 10000;
    }
  else //if no input by the user
    {
      cout << "assuming vector_size as 1000 and the grain_size as 10000" << endl;
      vector_size = 1000;
      grain_size = 10000;
    }
