...
/**************************************************************************
  ...
  Example     : VectVectMulGlobalMemSP.c
...
const char * vectVectMultGlobalMemKernelPath = "VectVectMultGlobalMemSP_kernel.cl";
int vectVectMultCheckResultGMSP (float *h_VectA,float *h_VectB,float output,int row);
...
void setExeEnvVectVectMultGMSP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *numPlatforms,cl_platform_id *selectedPlatform,cl_int *err)
{
	...
	*err=getPlatform(selectedPlatform,numDevices);
	...
	char* sProgramSource = readKernelSource(vectVectMultGlobalMemKernelPath);
...
/**********************************************************************
function to execute Vector Vector multiplication 
**********************************************************************/

void    vectorVectorMultiplicationGMSP (cl_uint numDevices,cl_device_id *devices, cl_program program,cl_context context,float * h_VectA,float *h_VectB, float *h_Output,int vectSize)
{
	cl_event                gpuExec[1];
        cl_int err;	
	...
	d_Output=clCreateBuffer(context,CL_MEM_WRITE_ONLY,sizeof(cl_float),NULL,&err);
	...
	/* reading buffer object*/
        err = clEnqueueReadBuffer(cmdQueue,d_Output,CL_TRUE,0,sizeof(cl_float),h_Output,0,0,&events);
	...
	/* calculate total gflops*/
        ...
        // Print the gflops on the screen
         print_on_screen("Vector Vector Multiplication using global memory",executionTimeInSeconds,vectSize,gflops,1);

	//check results 
	vectVectMultCheckResultGMSP(h_VectA,h_VectB,*h_Output,vectSize);
...
int main(int argc,char *argv[])
{
	...
	setExeEnvVectVectMultGMSP( &context , &numDevices, &devices, &program,&numPlatforms,&selectedPlatform,&err );


	//function to fo Vector Vector Multiplication	
	vectorVectorMultiplicationGMSP(numDevices, devices, program, context, h_VectA,h_VectB,&h_Output,vectSize);
...
int vectVectMultCheckResultGMSP (float *h_VectA,float *h_VectB,float output,int row)
{
	...
	int flag=0;
	/* sequential Vector Vector Multiplication*/
        for(i=0;i<row;i++)
        {
                //sum =0;
