...
/*****************************************************************************
  ...
  Example     : MatVectMulGlobalMemDP.c
 
  Objective   : Perform matrix-vectormultiplication using global memory
                (double precision)
...
const char * matVectMultDpGMKernelPath = "MatVectMultGlobalMemDP_kernel.cl";
int matVectMultCheckResultGMDP (double *h_Mat, double *h_Vect,double *output, int rows, int cols);
...
********************************************************************/
void setExeEnvMatVectMultGMDP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *numPlatforms,cl_platform_id *selectedPlatform,cl_int *err)
{
	...
	char* programSource = readKernelSource(matVectMultDpGMKernelPath);
...
*********************************************************************/
void    matrixVectorMultGMDP (cl_uint numDevices,cl_device_id *devices, cl_program program,cl_context context,double * h_Mat, double *h_Vect, double *h_Output,int height,int width)
{
	...
	/* calculate gflops*/
	 ...
	 print_on_screen("Matrix Vector Multiplication-double precision global memory",executionTimeInSeconds,height,gflops,1);
...
int main(int argc,char *argv[])
{
	...
	setExeEnvMatVectMultGMDP( &context , &numDevices, &devices, &program,&numPlatforms,&selectedPlatform,&err );

	//function to calculate Matrix Vector Multiplication
	matrixVectorMultGMDP(numDevices, devices, program, context, h_Mat,h_Vect, h_Output,height,width);


	//check opencl results with cpu results

	  matVectMultCheckResultGMDP( h_Mat,h_Vect,h_Output, height , width );
