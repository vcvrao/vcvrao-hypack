...
/*****************************************************************************
  ...
  Example     : MatVectMulGlobalMemSP.c
 
  Objective   : Perform matrix-vector multiplication using global memory
                (single precision)
...
const char * matVectMultGlobalMemKernelPath = "MatVectMultGlobalMemSP_kernel.cl";
int matVectMultCheckResultGMSP (float *h_Mat, float *h_Vect,float *output, int rows, int cols);
...
********************************************************************/
void setExeEnvMatVectMultGMSP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *numPlatforms,cl_platform_id *selectedPlatform,cl_int *err)
{
	...
	char* programSource = readKernelSource(matVectMultGlobalMemKernelPath);
...
*************************************************************/
void    matrixVectorMultGMSP (cl_uint numDevices,cl_device_id *devices, cl_program program,cl_context context,float * h_Mat, float *h_Vect, float *h_Output,int height,int width)
{
	...
	/* calculate gflops*/
	 ...
	 print_on_screen("Matrix Vector Multiplication-single precision global memory",executionTimeInSeconds,height,gflops,1);
...
int main(int argc,char *argv[])
{
	...
	setExeEnvMatVectMultGMSP( &context , &numDevices, &devices, &program,&numPlatforms,&selectedPlatform,&err );

	//function to calculate Matrix Vector Multiplication
	matrixVectorMultGMSP(numDevices, devices, program, context, h_Mat,h_Vect, h_Output,height,width);


	// check opencl results with cpu results

	 matVectMultCheckResultGMSP( h_Mat,h_Vect,h_Output, height , width );
