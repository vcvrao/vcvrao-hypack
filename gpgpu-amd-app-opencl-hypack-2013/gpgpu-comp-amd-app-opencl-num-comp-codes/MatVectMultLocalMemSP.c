/***********************************************************************
  ...
  Example     : MatVectMulLocalMemSP.c
...
const char * matVectMultKernelPath = "MatVectMultLocalMemSP_kernel.cl";
int matVectMultCheckResultLMSP (float *h_Mat, float *h_Vect,float *output, int rows, int cols);
...
********************************************************************/
void setExeEnvMatVectMultLMSP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *numPlatforms,cl_platform_id *selectedPlatform,cl_int *err)
{
	...
	char* sProgramSource = readKernelSource(matVectMultKernelPath);
...
*************************************************************/
void    matrixVectorMultLMSP (cl_uint numDevices,cl_device_id *devices, cl_program program,cl_context context,float * h_Mat, float *h_Vect, float *h_Output,int height,int width)
{
        ...
        size_t                  localWorkSize[1];    // holds global_work size
        int                      count;
        char                    dbuff[100];          
	...
	/** Create the command queue **/
        ...
        d_Vect =clCreateBuffer(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,width*sizeof(float),h_Vect,&err);
        OPENCL_CHECK_STATUS("Failed to create device input vector  ",err);

   	d_Output = clCreateBuffer ( context, CL_MEM_WRITE_ONLY ,height * sizeof(float),NULL, &err);
	...
	err = clSetKernelArg( kernel, 5, sizeof(cl_float)*BLOCK_SIZE*BLOCK_SIZE , 0);
        OPENCL_CHECK_STATUS( "Set  kernel argument 3 failed ",err);
	...
	/* total execuition time in seconds*/
 	executionTimeInSeconds = (float)(1.0e-9 * elapsedTime);
         ...
         //read the result
         err =clEnqueueReadBuffer(cmdQueue,d_Output,CL_TRUE,0,height*sizeof(cl_float),h_Output,0,0,&events);
	...
	/* calculate gflops*/
	 ...
	 print_on_screen("Matrix Vector Multiplication-single precision shared memory",executionTimeInSeconds,height,gflops,1);
...
int main(int argc,char *argv[])
{
	...
	float *h_Vect;		//holds host output buffer
	int i;
	int height=SIZE;
	int width=SIZE;
	...
	assert((h_Output=(float *)malloc(height*sizeof(float)))!=NULL);
	assert((h_Vect=(float *)malloc(width*sizeof(float)))!=NULL);

	/*initialize host memory*/
	fill_sp_matrix(h_Mat,height,width);
	fill_sp_vector(h_Vect,width);
	for(i=0;i<height;i++)
	{
		h_Output[i]=0;
	}
	...
	setExeEnvMatVectMultLMSP( &context , &numDevices, &devices, &program,&numPlatforms,&selectedPlatform,&err );

	//function to calculate Matrix Vector Multiplication
	matrixVectorMultLMSP(numDevices, devices, program, context, h_Mat,h_Vect, h_Output,height,width);


	// check opencl results with cpu results

	 ( matVectMultCheckResultLMSP( h_Mat,h_Vect,h_Output, height , width ));
...
*/	
	/* free the host memories*/
        hSpMatrixFree(h_Mat,height);
        hSpVectFree(h_Vect,width);
        hSpVectFree(h_Output,height);
}
...
int matVectMultCheckResultLMSP (float *h_Mat, float *h_Vect,float *output, int rows, int cols)
{
	...
	float  errorNorm = 0.0;
        ...
        assert((temp_Out = (float *)malloc( sizeof(float) * rows))!=NULL);
	colIndex = 0;
	/* sequential code to calculate Matrix Vector Multiplication */
	while(colIndex != rows)
	{
		tempResult = 0.0f;
