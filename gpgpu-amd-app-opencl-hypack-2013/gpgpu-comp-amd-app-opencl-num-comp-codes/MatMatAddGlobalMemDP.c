/**********************************************************************
  ...
  Example     : MatMatAddGlobalMemDP.c
 
  Objective   : Implement Matrix Matrix Addition using global memory
                (double precision)
...
const char * matMatAddGlobalMemKernelPath = "MatMatAddGlobalMemDP_kernel.cl";
...
**************************************************************/
void setExeEnvMatMatAddGMDP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *numPlatforms,cl_platform_id *selectedPlatform,cl_int *err)
{
	...
	*err=getPlatform(selectedPlatform,numDevices);
	...
	char* sProgramSource = readKernelSource(matMatAddGlobalMemKernelPath);
...
void    matrixMatrixAdditionGMDP (cl_uint numDevices,cl_device_id *devices, cl_program program,cl_context context,double * h_MatA,double *h_MatB, double *h_Output,int height,int width)
{
        cl_int err;
        cl_command_queue cmdQueue;   //holds command queue object
        cl_kernel kernel;               //holds kernel object
        cl_mem d_MatA,d_MatB,d_Output;            //holds device input output buffer
	size_t globalWorkSize[2]={height,width}; //holds global group size
	...
	//create command queue
        ...
        kernel = clCreateKernel(program,"MatMatAddKernelGMDP",&err);
        OPENCL_CHECK_STATUS("error while creating kernel",err);

        /*create buffer*/

        d_MatA=clCreateBuffer(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,sizeof(double)*(height*width),h_MatA,&err);
        OPENCL_CHECK_STATUS("error while creating buffer for input",err);
        
	d_MatB=clCreateBuffer(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,sizeof(double)*(height*width),h_MatB,&err);
        OPENCL_CHECK_STATUS("error while creating buffer for input",err);
        
	d_Output=clCreateBuffer(context,CL_MEM_WRITE_ONLY,sizeof(double)*(height*width),NULL,&err);
        OPENCL_CHECK_STATUS("error while creating buffer for d_Output",err);


        /*set kernel arg*/
        err=clSetKernelArg(kernel,0,sizeof(cl_mem),(void *)&d_MatA);
        OPENCL_CHECK_STATUS("error while setting arg 1",err);

        err=clSetKernelArg(kernel,1,sizeof(cl_mem),(void *)&d_MatB);
        OPENCL_CHECK_STATUS("error while setting arg 1",err);
	...
	/* total execution time*/
        ...
        err = clEnqueueReadBuffer(cmdQueue,d_Output,CL_TRUE,0,sizeof(cl_double)*height*width,h_Output,0,0,&events);
        OPENCL_CHECK_STATUS("error while reading buffer",err);


	/* calculate gflops*/
	 gflops= (1.0e-9 * ((1.0 * height*width) / executionTimeInSeconds));


        // Print the gflops on the screen
         print_on_screen("Matrix Matrix Addition using global memoryDoublePrecision",executionTimeInSeconds,height,gflops,1);
...
***************************************************************/
int matMatAddCheckResultGMDP (double *h_MatA,double *h_MatB,double *output,int row,int col)
{
	...
	double  errorNorm = 0.0;
        ...
        assert((temp_Out = (double *)malloc( sizeof(double) * row*col))!=NULL);

        /*sequential Matrix Matrix Addition result*/
        for( j=0 ; j<row*col  ; j++)
        {
                temp_Out[j]= h_MatA[j] + h_MatB[j];
        }
...
int main(int argc,char *argv[])
{
	...
	cl_kernel kernel;		//holds kernel object
	double *h_MatA;			//holds host input buffer
	double *h_MatB;			//holds host input buffer
	...
	setExeEnvMatMatAddGMDP( &context , &numDevices, &devices, &program,&numPlatforms,&selectedPlatform,&err );

	//function to calculate Matrix Matrix addition
	matrixMatrixAdditionGMDP(numDevices, devices,program,context,h_MatA,h_MatB,h_Output,height,width);

	//check result
	matMatAddCheckResultGMDP(h_MatA,h_MatB,h_Output,height,width);
...
********************************************************/
	...
	/* free host memory*/
        hDpMatFree(h_MatA,height);
        hDpMatFree(h_MatB,height);
        hDpMatFree(h_Output,height);
}
