...
****************************************************************
   ...
   Objecive  : Implement Vector Vector Addition 
               using global memory
   Created    : August-2013
...
/*souce code to implement Vector Vector Addition using global memory*/
...
const char * vectVectAddGlobalMemKernelPath = "VectVectAddGlobalMemSP_kernel.cl";
int vectVectAddCheckResultGMSP (float *h_VectA,float *h_VectB,float *output,int ROW);
...
void setExeEnvVectVectAddGMSP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *numPlatforms,cl_platform_id *selectedPlatform,cl_int *err)
{
	...
	char* programSource = readKernelSource(vectVectAddGlobalMemKernelPath);
...
/**********************************************************************
function to execute Vector Vector addition
**********************************************************************/

void    vectorVectorAdditionGMSP (cl_uint numDevices,cl_device_id *devices, cl_program program,cl_context context,float * h_VectA,float *h_VectB, float *h_Output,int vectSize)
{
	cl_event                gpuExec[1];
        cl_int err;	
	...
	/*create kernel object*/
        kernel = clCreateKernel(program,"VectVectAddKernel",&err);
	...
	/*create buffer*/
       ...
       d_VectB=clCreateBuffer(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,sizeof(float)*vectSize,h_VectB,&err);
        OPENCL_CHECK_STATUS("error while creating buffer for input",err);
	...
	/* calculate total gflops*/
        ...
        // Print the gflops on the screen
         print_on_screen("Vector Vector Addition using global memory",executionTimeInSeconds,vectSize,gflops,1);


	//check results 
	vectVectAddCheckResultGMSP(h_VectA,h_VectB,h_Output,vectSize);
...
int main(int argc, char *argv[])
{
	...
	float *h_VectB;			//holds host input buffer
	...
	setExeEnvVectVectAddGMSP( &context , &numDevices, &devices, &program,&numPlatforms,&selectedPlatform,&err );


	//function to fo Vector Vector Addition	
	vectorVectorAdditionGMSP(numDevices, devices, program, context, h_VectA,h_VectB,h_Output,vectSize);
...
int vectVectAddCheckResultGMSP (float *h_VectA,float *h_VectB,float *output,int ROW)
{
	 ...
	 double  errorNorm = 0.0;
        ...
        assert((temp_Out = (float *)malloc( sizeof(float) * ROW))!=NULL);
	/* sequential Vector Vector Addition*/
        for( j=0 ; j<ROW  ; j++)
        {
                temp_Out[j]= h_VectA[j] + h_VectB[j];
        }
	/* check opencl result with sequentioal result*/
        for( j=0 ; j < ROW  ; j++)
        {
