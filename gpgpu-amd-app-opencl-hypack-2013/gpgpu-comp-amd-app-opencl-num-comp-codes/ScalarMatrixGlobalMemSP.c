...
/************************************************************************
  ...
  Example     : ScalarMatrixGlobalMemSP.c
...
const char * scalarMatrixKernelPath = "ScalarMatrixGlobalMemSP_kernel.cl";
...
********************************************************************/
void setExeEnvScalarMatrixMulGMSP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *numPlatforms,cl_platform_id *selectedPlatform,cl_int *err)
{
...
*************************************************************/
void    scalarMatrixMulGMSP (cl_uint numDevices,cl_device_id *devices, cl_program program,cl_context context,float * h_Mat, int h_Scalar, float *h_Output,int height,int width)
{
	...
	/** Create the command queue **/
   	...
   	d_Output = clCreateBuffer ( context, CL_MEM_WRITE_ONLY ,(height*width)*sizeof(float),NULL, &err);
        ...
        OPENCL_CHECK_STATUS( "Failed to create device buffer d_cols   ",err);
         ...
         // Create the kernel
         kernel = clCreateKernel ( program, "scalarMatrixMult", &err);
	...
	/* calculate gflops*/
	 ...
	 print_on_screen("Scalar Matrix Multiplication-Single Precision Global Memory",executionTimeInSeconds,height,gflops,1);
...
******************************************************************/
int scalarMatrixMulCheckResultGMSP (float *h_Mat, int h_Scalar,float *output, int rows, int cols)
{
...
int main(int argc,char *argv[])
{
	...
	setExeEnvScalarMatrixMulGMSP( &context , &numDevices, &devices, &program,&numPlatforms,&selectedPlatform,&err );

	//function to calculate ScalR Matrix Multiplication
	scalarMatrixMulGMSP(numDevices, devices, program, context, h_Mat,h_Scalar, h_Output,height,width);


	// check opencl results with cpu results
	scalarMatrixMulCheckResultGMSP( h_Mat,h_Scalar,h_Output, height , width );
