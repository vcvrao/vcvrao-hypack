/****************************************************************
  ...
  Example     : PrefixSumGlobalMemDP.c
 
  Objective   : Calcualte prefix sum of an array of N elements 
                using global memory (Double precision)
...
const char * kernelSourcePath = "PrefixSumGlobalMemDP_kernel.cl";

int prefixSumCheckResultGMDP(double *inputArray,double *output,int arrayLength);
...
/* vector free*/
void hSpVectFree(double * arr,int len)
{
        int i;
        free(arr);
}
...
void setExeEnvPrefixSumGMDP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *numPlatforms,cl_platform_id *platforms,cl_int *err)
 {
...
void    prefixSumGMDP (cl_uint numDevices,cl_device_id *devices, cl_program program,cl_context context,double * h_Vect,double *h_Output,int vectSize)
{
        cl_event                gpuExec[1];
        cl_int err;
        ...
        /*create buffer*/
       d_Vect=clCreateBuffer(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,sizeof(double)*vectSize,h_Vect,&err);
        OPENCL_CHECK_STATUS("error while creating buffer for input",err);

       d_VectSize=clCreateBuffer(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,sizeof(double),(void *)&vectSize,&err);
        OPENCL_CHECK_STATUS("error while creating buffer for input",err);
...
int main(int argc, char *argv[])
 {
        ...
        size_t retValSize;
        double *h_Vect;                 //holds host input buffer
        ...
        assert((h_Vect=(double *)malloc(vectSize*sizeof(double)))!=NULL);
        assert((h_Output=(double *)malloc(vectSize*sizeof(double)))!=NULL);
        
        /*initialize host memory*/
        fill_dp_vector(h_Vect,vectSize);
        ...
        setExeEnvPrefixSumGMDP( &context , &numDevices, &devices, &program,&numPlatforms,&platforms,&err );


        //function to fo Vector Vector Addition 
        prefixSumGMDP (numDevices, devices, program, context, h_Vect,h_Output,vectSize);

	prefixSumCheckResultGMDP(h_Vect, h_Output,vectSize);
...
int prefixSumCheckResultGMDP(double *h_Vect,double *output,int arrayLength)
{
	...
	assert((temp_out = (double *)malloc( sizeof(double) * arrayLength))!=NULL);
