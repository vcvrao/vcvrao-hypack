/***************************************************************************
  ...
  Objective   : Perform matrix matrix multiplication using Global memory
                (single precision)
...
const char * matMatMultKernelPathGMSP = "MatMatMultGlobalMemSP_kernel.cl";
...
********************************************************************/
void setExeEnvMatMatMultGMSP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *num_platforms,cl_platform_id *selectedPlatform,cl_int *err)
{
	...
	char* programSource = readKernelSource(matMatMultKernelPathGMSP);
...
*************************************************************/
void    matrixMatrixMultGMSP (cl_uint numDevices,cl_device_id *devices, cl_program program,cl_context context,float * h_MatA, float *h_MatB, float *h_Output,int height,int width)
{
	...
	/* calculate gflops*/
	 ...
	 print_on_screen("Matrix Matrix Multiplication Global Memory Single Precision",executionTimeInSeconds,height,gflops,1);
...
******************************************************************/
int matMatMultCheckResultGMSP (float *h_MatA, float *h_MatB,float *output, int rows, int cols)
{
...
int main(int argc,char *argv[])
{
	...
	setExeEnvMatMatMultGMSP( &context , &numDevices, &devices, &program,&num_platforms,&selectedPlatform,&err );

	//function to calculate Matrix Matrix Multiplication
	matrixMatrixMultGMSP(numDevices, devices, program, context, h_MatA,h_MatB, h_Output,height,width);


	/*  check opencl results with cpu results*/

	  matMatMultCheckResultGMSP( h_MatA,h_MatB,h_Output, height , width );
