/***********************************************************************
  ...
  Example     : MatTransposeGlobalMemDP.c
 
  Objective   : Perform matrix transpose operation using global memory
                (double precision)
...
const char * transposeMatGlobalMemKernelPath = "MatTransposeGlobalMemDP_kernel.cl";
int matTransposeCheckResultGMDP(double *h_Mat,double *output,int rows,int cols);
...
**************************************************************/
void setExeEnvMatTransposeGMDP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *numPlatforms,cl_platform_id *selectedPlatforms,cl_int *err)
{
                                ...
                                *err = clGetDeviceIDs( platforms[count], CL_DEVICE_TYPE_GPU,0,0,numDevices);
                                ...
                                else
                                {
                                        ...
                                        *err = clGetDeviceIDs( platforms[count], CL_DEVICE_TYPE_GPU, (*numDevices), *devices, 0);
                                        /* get selected platform*/
                                        *selectedPlatforms=platforms[count];
                                        printf("\tPlatform used                            :  %s\n",pbuff);
                                        break;
                                }
                        }
...
void    matrixTransposeGMDP (cl_uint numDevices,cl_device_id *devices, cl_program program,cl_context context,double * h_Mat, double *h_Output,int height,int width)
{
        cl_int err;
	...
	/* total execution time*/
        ...
        // Print the gflops on the screen
         print_on_screen("Matrix Tranpose using global memoryDP",executionTimeInSeconds,height,gflops,0);
...
int main(int argc,char *argv[])
{
	...
	setExeEnvMatTransposeGMDP( &context , &numDevices, &devices, &program,&numPlatforms,&platforms,&err );

	//function to calculate Matrix Matrix addition
	matrixTransposeGMDP(numDevices, devices,program,context,h_Mat,h_Output,height,width);
	
	//check opencl result with sequential result
	matTransposeCheckResultGMDP(h_Mat,h_Output,height,width);
