/********************************************************************
  ...
  Example     : MatTransposeLocalMemDP.c
...
const char * transposeMatKernelPath = "MatTransposeLocalMemDP_kernel.cl";

int matTransposeCheckResultLMDP(double *h_Mat,double *output,int rows,int cols);
...
**************************************************************/
void setExeEnvMatTransposeLMDP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *numPlatforms,cl_platform_id *platforms,cl_int *err)
{
...
void    matrixTransposeLMDP (cl_uint numDevices,cl_device_id *devices, cl_program program,cl_context context,double * h_Mat, double *h_Output,int height,int width)
{
        cl_int err;
	...
	//create command queue
        ...
        kernel = clCreateKernel(program,"transMatrixDP",&err);
        OPENCL_CHECK_STATUS("error while creating kernel",err);

        /*create buffer*/

        d_Mat=clCreateBuffer(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,sizeof(double)*(height*width),h_Mat,&err);
        OPENCL_CHECK_STATUS("error while creating buffer for input",err);
      
	d_rows=clCreateBuffer(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,sizeof(int),(void *)&height,&err);
        OPENCL_CHECK_STATUS("error while creating buffer for input",err);
	...
	/* total execution time*/
        ...
        // Print the result  on the screen
         print_on_screen("Matrix Transpose using shared memoryDP ",executionTimeInSeconds,height,gflops,0);
...
int main(int argc,char *argv[])
{
	...
	setExeEnvMatTransposeLMDP( &context , &numDevices, &devices, &program,&numPlatforms,&platforms,&err );

	//function to calculate Matrix Matrix addition
	matrixTransposeLMDP(numDevices, devices,program,context,h_Mat,h_Output,height,width);

	/* check opencl result with sequential result */
	matTransposeCheckResultLMDP(h_Mat,h_Output,height,width);
