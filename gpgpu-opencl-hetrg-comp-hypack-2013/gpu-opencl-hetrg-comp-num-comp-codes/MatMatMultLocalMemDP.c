...
const char * matMatMultKernelPath = "MatMatMultLocalMemDP_kernel.cl";
...
********************************************************************/
void setExeEnvMatMatMultLMDP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *numPlatforms,cl_platform_id *selectedPlatform,cl_int *err)
{
...
*************************************************************/
void    matrixMatrixMultLMDP (cl_uint numDevices,cl_device_id *devices, cl_program program,cl_context context,double *h_MatA, double *h_MatB, double *h_Output,int height,int width)
{
        ...
        float                  total_time=0.0f; //holds total time taken for execution
	...
	/** Create the command queue **/
        ...
        /* create buffers*/
        d_MatA =clCreateBuffer(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,(height*width)*sizeof(double),h_MatA,&err);
        OPENCL_CHECK_STATUS("Failed to create device Mat buffer A  ",err);

        
        d_MatB =clCreateBuffer(context,CL_MEM_READ_ONLY|CL_MEM_COPY_HOST_PTR,(height*width)*sizeof(double),h_MatB,&err);
   	...
   	d_Output = clCreateBuffer ( context, CL_MEM_WRITE_ONLY ,( height*width) * sizeof(double),NULL, &err);
        ...
        OPENCL_CHECK_STATUS( "Failed to create device output  buffer   ",err);
         ...
         // Create the kernel
         kernel = clCreateKernel ( program, "matMatMultKernelDP", &err);
	...
	err = clSetKernelArg( kernel, 3, sizeof(cl_double) * BLOCK_SIZE * BLOCK_SIZE, 0);
        OPENCL_CHECK_STATUS( "Set  kernel argument 3 failed ",err);
           
	err = clSetKernelArg( kernel, 4, sizeof(cl_double) * BLOCK_SIZE * BLOCK_SIZE, 0);
	...
	/* calculate gflops*/
        gflops= (1.0e-9 * ((2.0 * height*height*height) / executionTimeInSeconds));


        // Print the gflops on the screen
	 print_on_screen("Matrix Matrix Multiplication Local Memory Double Precision",executionTimeInSeconds,height,gflops,1);
...
******************************************************************/
int matMatMultCheckResultLMDP (double *h_MatA, double *h_MatB,double *output, int rows, int cols)
{
	...
	double  errorNorm = 0.0;
        ...
        int     flag=0;
        assert((temp_Out = (double *)malloc( sizeof(double) * rows * rows))!=NULL);
...
int main(int argc,char *argv[])
{
	...
	double *h_MatA;			//holds host MatA buffer
	double *h_Output;		//holds host output buffer
	double *h_MatB;		//holds host output buffer
	int i;
	int height=SIZE;
	int width=SIZE;
	
	/* allocate host memory*/
	assert((h_MatA=(double *)malloc(height*width*sizeof(double)))!=NULL);
	assert((h_Output=(double *)malloc(height*width*sizeof(double)))!=NULL);
	assert((h_MatB=(double *)malloc(height*width*sizeof(double)))!=NULL);

	/*initialize host memory*/
	fill_dp_matrix(h_MatA,height,width);
	fill_dp_matrix(h_MatB,height,width);
	...
	setExeEnvMatMatMultLMDP( &context , &numDevices, &devices, &program,&numPlatforms,&selectedPlatform,&err );

	//function to calculate Matrix Matrix Multiplication
	matrixMatrixMultLMDP(numDevices, devices, program, context, h_MatA,h_MatB, h_Output,height,width);


	/*  check opencl results with cpu results*/

	 matMatMultCheckResultLMDP( h_MatA,h_MatB,h_Output, height , width );
...
/*	printf("MatA\n");
	...
	/* free the host memories*/
        hDpMatrixFree(h_MatA,height);
        hDpMatrixFree(h_MatB,height);
