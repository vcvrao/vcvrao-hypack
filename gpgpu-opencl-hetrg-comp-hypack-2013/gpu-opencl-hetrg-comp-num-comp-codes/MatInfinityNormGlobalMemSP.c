/***************************************************************
  ...
  Example     : MatInfinityNormGlobalMemSP.c
...
const char * matInfinityNormKernelPath = "MatInfinityNormGlobalMemSP_kernel.cl";
...
int matInfinityNormCheckResultGMSP(float *h_Mat,int rows,int cols,float *finalSum);
...
********************************************************************/
void setExeEnvMatInfinityGMSP(cl_context *context, cl_uint *numDevices, cl_device_id **devices, cl_program *program,cl_uint *numPlatforms,cl_platform_id *platforms,cl_int *err)
{
...
int main(int argc,char *argv[])
{
	...
	cl_command_queue queueGpu;   //holds command queue object
	...
	h_Mat=(float *)malloc(height*width*sizeof(float));
	h_Rowcol=(int *)malloc(2*sizeof(int));
	h_InfiNorm=(float *)malloc(sizeof(float));
	h_Rowcol[0] = height;
  	h_Rowcol[1] = width;
	...
	setExeEnvMatInfinityGMSP( &context , &numDevices, &devices, &program,&numPlatforms,&platforms,&err );

	//function to calculate Matrix Matrix Multiplication
	matInfinityNormGMSP(numDevices, devices, program, context, h_Mat,h_Rowcol,h_InfiNorm ,height,width);

	matInfinityNormCheckResultGMSP(h_Mat,height,width,h_InfiNorm);


	/* free the host memories*/
        hSpMatFree(h_Mat,height);
        hSpVectFree(h_InfiNorm,1);
	return 0;
}
...
******************************************************************/
int matInfinityNormCheckResultGMSP(float *h_Mat,int rows,int cols,float *cal_norm)
{
	int currRow,iCol,flag=0;
	float temp_norm =0.0f,myRowSum=0.0f;
