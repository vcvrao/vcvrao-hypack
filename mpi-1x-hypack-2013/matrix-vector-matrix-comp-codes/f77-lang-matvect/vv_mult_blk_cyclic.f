...
c
c  Example 5.3		: vv_mult_blk_cyclic.f
c
c  Objective           : Vector_Vector Multiplication (Using Cyclic data 
c                        Partitioning ) 
c
...
c*******************************************************************
	 ...
	 real	 	MyFinalVector, FinalAnswer
  	 ...
  	 real 		Buffer_A(DATA_SIZE), Buffer_B(DATA_SIZE)
...
C    ....... Rearrangement of data for Cyclic distribution ....... 
	if(MyRank .eq. Root) then
	 index = 1
      	 Block_Size = VectorSize / Numprocs
 	 do iproc = 1, Numprocs
	      do global_index  = iproc, VectorSize, Numprocs
...
C    ...... Scatter vector A and B ......
       ...
       call MPI_SCATTER(Buffer_A,ScatterSize,MPI_REAL,Mybuffer_A, 
     $		 ScatterSize,MPI_REAL,Root,MPI_COMM_WORLD, ierror)
			 
       call MPI_SCATTER( Buffer_B,ScatterSize,MPI_REAL,Mybuffer_B, 
     $		 ScatterSize,MPI_REAL,Root,MPI_COMM_WORLD,ierror)
